...
namespace Fryz.Apps.SpaceTrader
{
	public class Game : STSerializableObject
	{
		...
		private SpaceTraderWPF.SpaceTrader     _parentWin									= null;
		...
		public Game(string name, Difficulty difficulty, int pilot, int fighter, int trader, int engineer, SpaceTraderWPF.SpaceTrader parentWin)
		{
			_game				= this;
		...
		public Game(Hashtable hash, SpaceTraderWPF.SpaceTrader parentWin ): base(hash)
		{
			_game				= this;
			_parentWin	= parentWin;
		...
		public void Arrested()
		{
			...
			FormAlert.Alert(AlertType.EncounterArrested, ParentWin32Window);

			FormAlert.Alert(AlertType.JailConvicted, ParentWin32Window, Functions.Multiples(term, Strings.TimeUnit),
			...
			if (Commander.Ship.HasGadget(GadgetType.HiddenCargoBays))
			{
				...
				FormAlert.Alert(AlertType.JailHiddenCargoBaysRemoved, ParentWin32Window);
			}

			if (Commander.Ship.ReactorOnBoard)
			{
				FormAlert.Alert(AlertType.ReactorConfiscated, ParentWin32Window);
				QuestStatusReactor		= SpecialEvent.StatusReactorNotStarted;
			}

			if (Commander.Ship.SculptureOnBoard)
			{
				FormAlert.Alert(AlertType.SculptureConfiscated, ParentWin32Window);
				QuestStatusSculpture	= SpecialEvent.StatusSculptureNotStarted;
			}

			if (Commander.Ship.WildOnBoard)
			{
				FormAlert.Alert(AlertType.WildArrested, ParentWin32Window);
			...
			{
				FormAlert.Alert(AlertType.JailIllegalGoodsImpounded, ParentWin32Window);
				Commander.Ship.RemoveIllegalGoods();
			}

			if (Commander.Insurance)
			{
				FormAlert.Alert(AlertType.JailInsuranceLost, ParentWin32Window);
			...
			{
				FormAlert.Alert(AlertType.JailMercenariesLeave, ParentWin32Window);
			...
			else
			{
				...
				FormAlert.Alert(AlertType.JailShipSold, ParentWin32Window);

				if (Commander.Ship.Tribbles > 0)
					FormAlert.Alert(AlertType.TribblesRemoved, ParentWin32Window);

				FormAlert.Alert(AlertType.FleaBuilt, ParentWin32Window);
				CreateFlea();
			}
			...
			IncDays(term, ParentWin32Window);
		}
		...
		private void ArrivalCheckDebt()
		{
			...
			if (Commander.Debt >= Consts.DebtWarning)
				FormAlert.Alert(AlertType.DebtWarning, ParentWin32Window);
				// Debt Reminder
			else if (Commander.Debt > 0 && Options.RemindLoans && Commander.Days % 5 == 0)
				FormAlert.Alert(AlertType.DebtReminder, ParentWin32Window, Functions.Multiples(Commander.Debt, Strings.MoneyUnit));
		}
		...
		private void ArrivalCheckEasterEgg()
		{
			...
			if (Commander.CurrentSystem.Id == StarSystemId.Og)
			{
				...
				{
					FormAlert.Alert(AlertType.Egg, ParentWin32Window);
		...
		private void ArrivalCheckReactor()
		{
			...
			{
				FormAlert.Alert(AlertType.ReactorMeltdown, ParentWin32Window);
				QuestStatusReactor	= SpecialEvent.StatusReactorNotStarted;
				if (Commander.Ship.EscapePod)
					EscapeWithPod();
				else
				{
					FormAlert.Alert(AlertType.ReactorDestroyed, ParentWin32Window);
			...
			else
			{
				...
				if (QuestStatusReactor == SpecialEvent.StatusReactorFuelOk + 1)
					FormAlert.Alert(AlertType.ReactorWarningFuel, ParentWin32Window);
					// better deliver it soon!
				else if (QuestStatusReactor == SpecialEvent.StatusReactorDate - 4)
					FormAlert.Alert(AlertType.ReactorWarningFuelGone, ParentWin32Window);
					// last warning!
				else if (QuestStatusReactor == SpecialEvent.StatusReactorDate - 2)
					FormAlert.Alert(AlertType.ReactorWarningTemp, ParentWin32Window);
			}
		}
		...
		private void ArrivalCheckTribbles()
		{
			...
			if (ship.Tribbles > 0)
			{
				...
				if (ship.ReactorOnBoard)
				{
					...
					{
						ship.Tribbles	= 0;
						FormAlert.Alert(AlertType.TribblesAllDied, ParentWin32Window);
					}
					else
					{
						ship.Tribbles	/= 2;
						FormAlert.Alert(AlertType.TribblesHalfDied, ParentWin32Window);
					}
				}
				else if (ship.Cargo[narc] > 0)
				{
					...
					FormAlert.Alert(AlertType.TribblesMostDied, ParentWin32Window);
				}
				else
				{
					if (ship.Cargo[food] > 0 && ship.Tribbles < Consts.MaxTribbles)
					{
						...
						FormAlert.Alert(AlertType.TribblesAteFood, ParentWin32Window);
					}
						...
						(previousTribbles < Consts.MaxTribbles && ship.Tribbles == Consts.MaxTribbles))
					{
						...
						FormAlert.Alert(AlertType.TribblesInspector, ParentWin32Window, qty);
					}
				}
		...
		private void ArrivalPerformRepairs()
		{
			...
			if (!fuelOk && !repairOk)
				FormAlert.Alert(AlertType.ArrivalIFFuelRepairs, ParentWin32Window);
			else if (!fuelOk)
				FormAlert.Alert(AlertType.ArrivalIFFuel, ParentWin32Window);
			else if (!repairOk)
				FormAlert.Alert(AlertType.ArrivalIFRepairs, ParentWin32Window);
		}
		...
		{
			FormAlert.Alert(AlertType.EncounterEscapePodActivated, ParentWin32Window);

			if (Commander.Ship.SculptureOnBoard)
				FormAlert.Alert(AlertType.SculptureSaved, ParentWin32Window);
			...
			if (Commander.Ship.Tribbles > 0)
				FormAlert.Alert(AlertType.TribblesKilled, ParentWin32Window);

			if (QuestStatusJapori == SpecialEvent.StatusJaporiInTransit)
			{
				int	system;
				for (system = 0; system < Universe.Length && Universe[system].SpecialEventType != SpecialEventType.Japori; system++);
				FormAlert.Alert(AlertType.AntidoteDestroyed, ParentWin32Window, Universe[system].Name);
				QuestStatusJapori = SpecialEvent.StatusJaporiNotStarted;
			}

			if (Commander.Ship.ArtifactOnBoard)
			{
				FormAlert.Alert(AlertType.ArtifactLost, ParentWin32Window);
			...
			if (Commander.Insurance)
			{
				FormAlert.Alert(AlertType.InsurancePayoff, ParentWin32Window);
		...
		public void HandleSpecialEvent()
		{
			...
			switch (curSys.SpecialEventType)
			{
				...
				case SpecialEventType.CargoForSale:
					FormAlert.Alert(AlertType.SpecialSealedCanisters, ParentWin32Window);
				...
				case SpecialEventType.DragonflyShield:
					...
					{
						FormAlert.Alert(AlertType.EquipmentNotEnoughSlots, ParentWin32Window);
						remove	= false;
					}
					else
					{
						FormAlert.Alert(AlertType.EquipmentLightningShield, ParentWin32Window);
						Commander.Ship.AddEquipment(Consts.Shields[(int)ShieldType.Lightning]);
						QuestStatusDragonfly	= SpecialEvent.StatusDragonflyDone;
					}
					break;
				case SpecialEventType.EraseRecord:
					FormAlert.Alert(AlertType.SpecialCleanRecord, ParentWin32Window);
				...
				case SpecialEventType.GemulonFuel:
					...
					{
						FormAlert.Alert(AlertType.EquipmentFuelCompactor, ParentWin32Window);
				...
				case SpecialEventType.Japori:
					...
					if (Commander.Ship.FreeCargoBays < 10)
						FormAlert.Alert(AlertType.CargoNoEmptyBays, ParentWin32Window);
					else
					{
						FormAlert.Alert(AlertType.AntidoteOnBoard, ParentWin32Window);
				...
				case SpecialEventType.Jarek:
					...
					{
						FormAlert.Alert(AlertType.SpecialNoQuarters, ParentWin32Window);
						remove	= false;
					}
					else
					{
						CrewMember	jarek	= Mercenaries[(int)CrewMemberId.Jarek];
						FormAlert.Alert(AlertType.SpecialPassengerOnBoard, ParentWin32Window, jarek.Name);
				...
				case SpecialEventType.Moon:
					FormAlert.Alert(AlertType.SpecialMoonBought, ParentWin32Window);
				...
				case SpecialEventType.PrincessQonos:
					...
					else
					{
						...
						FormAlert.Alert(AlertType.SpecialPassengerOnBoard, ParentWin32Window, princess.Name);
				...
				case SpecialEventType.PrincessQuantum:
					...
					{
						FormAlert.Alert(AlertType.EquipmentQuantumDisruptor, ParentWin32Window);
				...
				case SpecialEventType.Reactor:
					...
					else
					{
						if (Commander.Ship.WildOnBoard)
						{
							if (FormAlert.Alert(AlertType.WildWontStayAboardReactor, ParentWin32Window, curSys.Name) ==
								DialogResult.OK)
							{
								FormAlert.Alert(AlertType.WildLeavesShip, ParentWin32Window, curSys.Name);
								QuestStatusWild	= SpecialEvent.StatusWildNotStarted;
							}
							else
								remove	= false;
						...
						if (remove)
						{
							FormAlert.Alert(AlertType.ReactorOnBoard, ParentWin32Window);
				...
				case SpecialEventType.ReactorLaser:
					...
					{
						FormAlert.Alert(AlertType.EquipmentMorgansLaser, ParentWin32Window);
				...
				case SpecialEventType.ScarabUpgradeHull:
					FormAlert.Alert(AlertType.ShipHullUpgraded, ParentWin32Window);
				...
				case SpecialEventType.SculptureHiddenBays:
					...
					{
						FormAlert.Alert(AlertType.EquipmentHiddenCompartments, ParentWin32Window);
						Commander.Ship.AddEquipment(Consts.Gadgets[(int)GadgetType.HiddenCargoBays]);
						QuestStatusSculpture	= SpecialEvent.StatusSculptureDone;
					}
					break;
				case SpecialEventType.Skill:
					FormAlert.Alert(AlertType.SpecialSkillIncrease, ParentWin32Window);
					Commander.IncreaseRandomSkill();
					break;
				...
				case SpecialEventType.Tribble:
					FormAlert.Alert(AlertType.TribblesOwn, ParentWin32Window);
					Commander.Ship.Tribbles	= 1;
					break;
				case SpecialEventType.TribbleBuyer:
					FormAlert.Alert(AlertType.TribblesGone, ParentWin32Window);
				...
				case SpecialEventType.Wild:
					...
					{
						FormAlert.Alert(AlertType.WildWontBoardLaser, ParentWin32Window);
						remove	= false;
					}
					else if (Commander.Ship.ReactorOnBoard)
					{
						FormAlert.Alert(AlertType.WildWontBoardReactor, ParentWin32Window);
						remove	= false;
					}
					else
					{
						CrewMember	wild	= Mercenaries[(int)CrewMemberId.Wild];
						FormAlert.Alert(AlertType.SpecialPassengerOnBoard, ParentWin32Window, wild.Name);
						Commander.Ship.Hire(wild);
						QuestStatusWild	= SpecialEvent.StatusWildStarted;

						if (Commander.Ship.SculptureOnBoard)
							FormAlert.Alert(AlertType.WildSculpture, ParentWin32Window);
					}
					break;
		...
		public void ShowNewspaper()
		{
			if (!PaidForNewspaper)
			{
				...
				if (Commander.Cash < cost)
					FormAlert.Alert(AlertType.ArrivalIFNewspaper, ParentWin32Window, Functions.Multiples(cost, "credit"));
				else if (Options.NewsAutoPay || FormAlert.Alert(AlertType.ArrivalBuyNewspaper, ParentWin32Window,
			...
			if (PaidForNewspaper)
				FormAlert.Alert(AlertType.Alert, ParentWin32Window, NewspaperHead, NewspaperText);
		}
		...
		public bool Travel()
		{
			...
			{
				FormAlert.Alert(AlertType.SpecialTimespaceFabricRip, ParentWin32Window);
			...
			while (Clicks > 0)
			{
				...
				if (DetermineEncounter())
				{
					...
					form.ShowDialog(ParentWin32Window);
		...
		public void Warp(bool viaSingularity)
		{
			if (Commander.Debt > Consts.DebtTooLarge)
				FormAlert.Alert(AlertType.DebtTooLargeGrounded, ParentWin32Window);
			else if (Commander.Cash < MercenaryCosts)
				FormAlert.Alert(AlertType.LeavingIFMercenaries, ParentWin32Window);
			else if (Commander.Cash < MercenaryCosts + InsuranceCosts)
				FormAlert.Alert(AlertType.LeavingIFInsurance, ParentWin32Window);
			else if (Commander.Cash < MercenaryCosts + InsuranceCosts + WormholeCosts)
				FormAlert.Alert(AlertType.LeavingIFWormholeTax, ParentWin32Window);
			else
			{
				bool	wildOk	= true;
				...
				{
					if (FormAlert.Alert(AlertType.WildWontStayAboardLaser, ParentWin32Window, Commander.CurrentSystem.Name) ==
						DialogResult.Cancel)
						wildOk	= false;
					else
					{
						FormAlert.Alert(AlertType.WildLeavesShip, ParentWin32Window, Commander.CurrentSystem.Name);
						QuestStatusWild	= SpecialEvent.StatusWildNotStarted;
					}
				}
				...
				if (wildOk)
				{
					...
					if (Travel())
					{
						...
						/*
						if (Clicks == 0)
							FormAlert.Alert(AlertType.TravelArrival, ParentWin32Window);
						*/
					}
					else
						FormAlert.Alert(AlertType.TravelUneventfulTrip, ParentWin32Window);
      ...
      public System.Windows.Forms.IWin32Window ParentWin32Window = new System.Windows.Forms.NativeWindow();


      public SpaceTraderWPF.SpaceTrader		ParentWindow
		{
			get
			{
				return _parentWin;
			...
			{
            _parentWin = value;
            ( ParentWin32Window as System.Windows.Forms.NativeWindow ).AssignHandle( new System.Windows.Interop.WindowInteropHelper( _parentWin ).Handle );
         }
      }
